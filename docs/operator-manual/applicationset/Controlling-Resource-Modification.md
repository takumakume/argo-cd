...
To allow the ApplicationSet controller to create or modify `Application` resources, but prevent Applications from being updating, add the following parameter to the ApplicationSet controller `Deployment`:
```
